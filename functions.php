<?php
/**
 * mintheme functions and definitions
...
if ( ! function_exists( 'mintheme_setup' ) ) :
	...
	function mintheme_setup() {
		 ...
		 * If you're building a theme based on mintheme, use a find and replace
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'mintheme' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'mintheme_custom_background_args',
				array(
...
function mintheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'mintheme' ),
			'id'            => 'sidebar-1',
...
function mintheme_scripts() {
	wp_enqueue_style( 'mintheme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'mintheme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'mintheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	wp_enqueue_script( 'mintheme-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), _S_VERSION, true );
